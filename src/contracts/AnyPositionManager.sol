 ...
 */
contract AnyPositionManager is UnlockingHook, FeeDistributor, StoreKeys, IAnyPositionManager {
    ...
    function flaunch(
        ...
        // Initialize our memecoin with the sqrtPriceX96 and cache the resulting tick
        int24 initialTick =
            poolManager.initialize(_poolKey, initialPrice.getSqrtPriceX96(msg.sender, currencyFlipped, _params.initialPriceParams));

        // [F-1] Seed the price Oracle's ring buffer with the initial tick so that the very first
        ...
        oracle.recordObservation(poolId, initialTick);
    ...
    function _beforeSwap(
        ...
        (uint tokenIn, uint tokenOut) = internalSwapPool.internalSwap(_key, _params, nativeIsZero);
        if (tokenIn + tokenOut != 0) {
            // The {InternalSwapPool} computed the fill against its inventory; as the swap-delta
            // owner this hook performs the matching take (native) / settle (memecoin).
            poolManager.take(nativeIsZero ? _key.currency0 : _key.currency1, address(this), tokenIn);
            (nativeIsZero ? _key.currency1 : _key.currency0).settle(poolManager, address(this), tokenOut, false);
        ...
        // [F-1] Record the pre-swap tick into the price oracle (at most once per block) on every
        // swap, regardless of direction or inventory, so the TWAP reference stays fresh.
        oracle.recordObservation(_key.toId(), _beforeSwapTick);
    ...
    function _closeBidWall(
        ...
        poolManager.unlock(abi.encode(_poolKey));
    }

    /**
     * Overrides the creator lookup, as an imported ERC20 has no `creator` call of its own. The
     * holder of the {AnyFlaunch} ERC721 is instead resolved from the flaunch contract.
     *
    ...
    function _memecoinCreator(
        ...
        return flaunchContract.creator(_memecoin);
    }

    /**
     * Dispatched from {UnlockingHook.unlockCallback} (which has already verified the caller
     * is the {PoolManager}). Currently only handles the `{BidWall}.closeBidWall` action.
     *
    ...
    function _distributeFees(
        ...
        // creator that fees should route to. Read `flaunchContract.creator(memecoin)` live at
        // each decision point rather than caching once across external calls. Defends against
        // any future call-path that could change ownership between branches.

        if (creatorFee != 0) {
            address creatorNow = _memecoinCreator(memecoin);
        ...
        if (treasuryFee != 0) {
            ...
            if (_memecoinCreator(memecoin) != address(0)) {
    ...
    function _emitSwapUpdate(
        ...
        // Emit the Uniswap V4 standardised event. We can just send the ISP amounts directly.
