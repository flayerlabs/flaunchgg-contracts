...
library HookMiner {
    ...
    /// `address(this)` or the pranking address In `forge script`, this should be
    ...
    /// `abi.encode(address(manager))` @return hookAddress salt and corresponding address that was found. The salt can
    /// be used in `new Hook{salt: salt}(<constructor arguments>)`
    function find(
        address deployer,
        uint160 flags,
    ...
    function computeAddress(
        ...
        return address(uint160(uint(keccak256(abi.encodePacked(bytes1(0xFF), deployer, salt, keccak256(creationCode))))));
    }
}
